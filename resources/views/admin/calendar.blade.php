    ...
    <aside class="fixed left-0 top-0 h-full w-64 bg-white shadow-lg">
        <div class="p-6">
            ...
            <nav class="space-y-2">
                ...
                <a href="{{ route('admin.calendar') }}" class="flex items-center gap-3 px-4 py-3 bg-gray-100 text-gray-800 rounded-lg font-medium">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="flex items-center gap-6 mt-4 pt-4 border-t border-gray-100">
                <div class="flex items-center gap-2 text-sm text-gray-600">
                    <div class="w-3 h-3 rounded-full bg-green-500"></div> Semua selesai
                </div>
                <div class="flex items-center gap-2 text-sm text-gray-600">
...
<div id="detailModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[80vh] flex flex-col">
        ...
        <div id="modalContent" class="overflow-y-auto p-6 space-y-4"></div>
    </div>
</div>
...
function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        let dotHtml = '';
        if (tasks.length > 0) {
            const allDone = tasks.every(t => t.status === 'Selesai');
            const color = allDone ? 'bg-green-500' : 'bg-red-500';
            dotHtml = `<div class="w-1.5 h-1.5 rounded-full ${color} mx-auto mt-1"></div>`;
        }
...
function openDay(dateStr) {
    ...
    if (tasks.length === 0) {
        ...
        let html = '';
        for (const [userName, userTasks] of Object.entries(byUser)) {
            const allDone = userTasks.every(t => t.status === 'Selesai');
            const badgeClass = allDone ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700';
            const badgeText = allDone ? 'Semua Selesai' : 'Belum Selesai';

            html += `
                <div class="border border-gray-100 rounded-xl p-4">
                    <div class="flex justify-between items-center mb-3">
                        <div class="flex items-center gap-2">
                            <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(userName)}&background=e5e7eb&color=374151&size=32" class="w-8 h-8 rounded-full">
                            <span class="font-semibold text-gray-800">${userName}</span>
                        </div>
                        <span class="text-xs px-2 py-1 rounded-full font-medium ${badgeClass}">${badgeText}</span>
                    </div>
                    <div class="space-y-2">
            `;
            ...
            userTasks.forEach(t => {
                ...
                html += `
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="text-xs px-2 py-0.5 rounded-full ${statusClass}">${t.status}</span>
                    </div>
                `;
            });
