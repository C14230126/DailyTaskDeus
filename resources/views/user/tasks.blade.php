            ...
            <form action="{{ route('user.tasks') }}" method="GET" class="mb-6">
                ...
                <div class="flex gap-4 mt-4 items-center">
                    ...
                    <div class="flex gap-2">
                        <button type="button" class="px-4 py-2.5 text-sm text-gray-600 hover:text-gray-800">
                            Filter aktif
                        </button>
                        @if(request('date'))
                        <span class="px-3 py-2 bg-blue-100 text-blue-800 text-sm rounded-lg flex items-center gap-2">
                            {{ \Carbon\Carbon::parse(request('date'))->format('d/m/Y') }}
                            <button type="button" onclick="window.location.href='{{ route('user.tasks', array_diff_key(request()->all(), ['date' => ''])) }}'" 
                        ...
                        <a href="{{ route('user.tasks') }}" 
                           class="px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition text-sm">
                            Reset Semua Filter
                        </a>
                    </div>
                </div>
            ...
            <div class="space-y-3">
                ...
                <div class="border-l-4 p-4 bg-white rounded-lg hover:shadow-md transition
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <h5 class="font-bold text-gray-800 mb-1">{{ $task->title }}</h5>
